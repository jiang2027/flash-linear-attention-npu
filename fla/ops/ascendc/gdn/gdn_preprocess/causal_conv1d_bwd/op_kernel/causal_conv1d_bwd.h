...
private:
    ...
    __aicore__ inline void CopyInInputTile(GlobalTensor<inputT> &srcGm, LocalTensor<float> dst,
                                           ...
                                           uint32_t totalRows, uint32_t seqLen);
    ...
    __aicore__ inline void AtomicAddDwDb(uint32_t i_d);
    ...
    GlobalTensor<inputT> xGm_, yGm_, weightGm_, dyGm_, initialStateGm_, dhtGm_, dxGm_, dh0Gm_;
    GlobalTensor<int64_t> queryStartLocGm_;
    GlobalTensor<float> dwGm_, dbGm_;
    ...
    GM_ADDR dx, GM_ADDR dw, GM_ADDR db, GM_ADDR dh0, GM_ADDR workspace)
{
    ...
    dwGm_.SetGlobalBuffer(reinterpret_cast<__gm__ float *>(dw));
...
__aicore__ inline void CausalConv1dBwdKernel<inputT, calT>::CopyInInputTile(
    ...
    uint32_t i_d, uint32_t totalRows, uint32_t seqLen)
{
    ...
    if constexpr (IsSameType<inputT, float>::value) {
        while (remain > 0) {
            uint32_t segLen = GetInputSegmentLen(channel, remain);
            uint32_t rowStride = GetInputRowStride(channel);
            DataCopyExtParams copyParams{
...
{
    CopyInInputTile(xGm_, xBuf_.Get<float>(), bos, i_t * BT_, i_d, BT_, seqLen);
}
...
{
    CopyInInputTile(dyGm_, dyBuf_.Get<float>(), bos, i_t * BT_ + i_w, i_d, BT_, seqLen);
}
...
__aicore__ inline void CausalConv1dBwdKernel<inputT, calT>::CopyInDyWindow(
    uint64_t bos, uint32_t i_t, uint32_t i_d, uint32_t seqLen)
{
    CopyInInputTile(dyGm_, dyBuf_.Get<float>(), bos, i_t * BT_, i_d, dyRows_, seqLen);
}
...
__aicore__ inline void CausalConv1dBwdKernel<inputT, calT>::CopyInYAndSiluBwd(
    uint64_t bos, uint32_t i_t, uint32_t i_d, uint32_t i_w, uint32_t seqLen)
{
    CopyInInputTile(yGm_, yBuf_.Get<float>(), bos, i_t * BT_ + i_w, i_d, BT_, seqLen);
}
...
__aicore__ inline void CausalConv1dBwdKernel<inputT, calT>::CopyOutDwDb(uint32_t i_d)
{
    ...
    if (hasWeight_) {
        LocalTensor<float> dwLocal = dwBuf_.Get<float>();
        DataCopyExtParams dwOutParams{static_cast<uint16_t>(W_),
            static_cast<uint32_t>(BD_ * FP32_DTYPE_SIZE), 0,
            static_cast<uint32_t>((D_ - BD_) * FP32_DTYPE_SIZE), 0};
    ...
    if (hasBias_) {
        ...
        DataCopyExtParams dbParams{1, static_cast<uint32_t>(BD_ * FP32_DTYPE_SIZE), 0, 0, 0};
...
template <typename inputT, typename calT>
__aicore__ inline void CausalConv1dBwdKernel<inputT, calT>::AtomicAddDwDb(uint32_t i_d)
{
    if (!hasWeight_ && !hasBias_) return;
