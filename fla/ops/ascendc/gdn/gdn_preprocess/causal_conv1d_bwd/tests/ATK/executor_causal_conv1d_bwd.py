...
def _query_start_loc_for_layout(input_data, input_layout):
    ...
    _, t, _, _, _ = _layout_meta(x, layout)
...
def _causal_conv1d_bwd_cpu_fixed(x, y, weight, dy, initial_state, dht, activation, input_dtype=None):
    b, t, d = x.shape
    w = weight.shape[0]
...
def causal_conv1d_bwd_cpu(x, y, weight, dy, initial_state, dht, query_start_loc, activation,
    ...
    layout = _normalize_layout(input_layout)
    input_dtype = input_dtype or x.dtype
    x_logic = _input_to_logical(x, layout)
    ...
    return dx.contiguous(), dw.contiguous(), db.contiguous(), dh0.contiguous()
...
class CausalConv1dBwdApi(BaseApi):
    ...
    def init_by_input_data(self, input_data: InputDataset):
        ...
        b, t, d, n_heads, head_dim = _layout_meta(x, input_layout)
        ...
        x_logic = _input_to_logical(x, input_layout)
