...
- 支持输入 layout 转换：`inputLayoutOptional` 只指示 `x`、`yOptional`、`dy` 的输入物理排布，输出 layout 保持当前算子的逻辑输出格式。
- 计算公式：
...
- 参数说明：
  ...
  | 参数名 | 输入/输出 | 描述 |
  | :-- | :--: | :-- |
  | x | 输入 | 前向输入 tensor。数据类型支持 `FLOAT`、`FLOAT16`、`BFLOAT16`，format 支持 `ND`。shape 由 `inputLayoutOptional` 决定，详见“Layout 与 shape 说明”。 |
...
- Layout 与 shape 说明：
  ...
  | inputLayoutOptional | 输入 `x/yOptional/dy` shape | 逻辑维度解释 | `dxOut` shape | `queryStartLocOptional` |
  | :-- | :-- | :-- | :-- | :-- |
  | `BSND` / `BSH` | `[B, T, D]` | `B` 为 batch size，`T` 为固定序列长度，`D` 为特征维度 | `[B, T, D]` | 可为 `nullptr` |
  | `BNSD` | `[B, N, T, Dh]` | `D=N*Dh` | `[B, T, D]` | 可为 `nullptr` |
  | `TND` | `[totalTokens, D]` | `totalTokens=sum(seqLen_i)`，`D` 为特征维度 | `[totalTokens, D]` | 必须提供 |
  | `NTD` | `[N, totalTokens, Dh]` | `D=N*Dh` | `[totalTokens, D]` | 必须提供 |
...
- 数据类型：
  ...
  - `dwOutOptional`、`dbOutOptional` 的数据类型必须为 `FLOAT`。
  - `FLOAT16`、`BFLOAT16` 场景下，算子内部按 `FLOAT` 进行主要累积计算，输出 `dxOut`/`dh0OutOptional` 再转换回输入数据类型。
- shape：
...
- layout：
  - `inputLayoutOptional` 仅影响 `x`、`yOptional`、`dy` 的输入物理排布。
...
### Q1：为什么 `dwOutOptional` 和 `dbOutOptional` 的数据类型必须是 `FLOAT`？

`dw` 和 `db` 需要跨 batch、序列长度和 block 进行累加，使用 `FLOAT` 可以降低低精度输入在累积阶段的精度损失。

### Q2：`BNSD`/`NTD` layout 下为什么 `dxOut` 不是同样的物理 layout？

`inputLayoutOptional` 只描述输入 `x/yOptional/dy` 的物理排布。当前反向算子保持已有输出接口不变：固定长度输出 `[B, T, D]`，变长输出 `[totalTokens, D]`。
