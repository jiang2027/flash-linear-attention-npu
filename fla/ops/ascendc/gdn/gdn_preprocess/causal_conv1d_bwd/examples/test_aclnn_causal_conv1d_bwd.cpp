...
int aclnnCausalConv1dBwdTest(int32_t deviceId, aclrtStream& stream) {
  ...
  std::vector<float> dwHostData(GetShapeSize(dwShape), 0.0f);
  std::vector<float> dbHostData(GetShapeSize(dbShape), 0.0f);
  ...
  // 创建dw aclTensor
  ret = CreateAclTensor(dwHostData, dwShape, &dwDeviceAddr, aclDataType::ACL_FLOAT, &dw);
  CHECK_FREE_RET(ret == ACL_SUCCESS, return ret);
  std::unique_ptr<aclTensor, aclnnStatus (*)(const aclTensor *)> dwTensorPtr(dw, aclDestroyTensor);

  // 创建db aclTensor
  ret = CreateAclTensor(dbHostData, dbShape, &dbDeviceAddr, aclDataType::ACL_FLOAT, &db);
  CHECK_FREE_RET(ret == ACL_SUCCESS, return ret);
  ...
  std::vector<float> dwData(dwSize, 0);
  ret = aclrtMemcpy(dwData.data(), dwData.size() * sizeof(float), dwDeviceAddr,
                    dwSize * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
  ...
  std::vector<float> dbData(dbSize, 0);
  ret = aclrtMemcpy(dbData.data(), dbData.size() * sizeof(float), dbDeviceAddr,
                    dbSize * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
...
int RunInputLayoutFp32Test(aclrtStream& stream, const std::string& layout) {
  ...
  std::vector<int64_t> xShape = isNtD ? std::vector<int64_t>{N, logicalT, H}
  ...
  std::vector<float> dyHost(GetShapeSize(dyShape));
  LogicalToPhysicalInput(xLogical, layout, logicalB, logicalT, N, H, xHost);
