...
- 支持输入 layout 转换：`inputLayout` 只影响 `x`、`y`、`dy` 的输入物理排布，输出仍保持逻辑 layout。
...
| :-- | :-- | :-- | :--: | :-- |
| x | FLOAT/FLOAT16/BF16 | 由 `inputLayout` 决定 | 必选 | 前向输入。 |
...
| inputLayout | string | BSND | BSND, BSH, TND, BNSD, NTD | 输入 `x/y/dy` 的物理 layout。 |

## Layout 与 Shape

| inputLayout | 输入 `x/y/dy` shape | 逻辑维度 | `dx` 输出 shape | `queryStartLoc` |
| :-- | :-- | :-- | :-- | :-- |
| `BSND` / `BSH` | `[B, T, D]` | `B` 为 batch size，`T` 为固定序列长度，`D` 为特征维度 | `[B, T, D]` | 可不传 |
| `BNSD` | `[B, N, T, Dh]` | `D=N*Dh` | `[B, T, D]` | 可不传 |
| `TND` | `[totalTokens, D]` | `totalTokens=sum(seqLen_i)` | `[totalTokens, D]` | 必须传 |
| `NTD` | `[N, totalTokens, Dh]` | `D=N*Dh` | `[totalTokens, D]` | 必须传 |
